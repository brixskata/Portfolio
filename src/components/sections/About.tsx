        ...
        <div className="grid gap-14 lg:grid-cols-[0.82fr_1.18fr] lg:gap-24">
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.7 }}>
            <div className="group relative overflow-hidden border border-zinc-800 bg-zinc-900">
              ...
              <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-gradient-to-t from-black/80 to-transparent px-4 pb-2 pt-12"><span className="font-mono text-[11px] text-zinc-400">PERSONAL ARCHIVE</span><div className="flex items-center gap-2"><span className="font-mono text-xs text-zinc-400">{String(photoIndex + 1).padStart(2, "0")} / {String(ABOUT_PHOTOS.length).padStart(2, "0")}</span><button type="button" onClick={showPreviousPhoto} aria-label="Show previous photo" className="p-1 text-zinc-400 transition hover:text-white focus-visible:outline-2 focus-visible:outline-blue-500"><HiChevronLeft size={18} /></button><button type="button" onClick={showNextPhoto} aria-label="Show next photo" className="p-1 text-zinc-400 transition hover:text-white focus-visible:outline-2 focus-visible:outline-blue-500"><HiChevronRight size={18} /></button></div></div>
            </div>
